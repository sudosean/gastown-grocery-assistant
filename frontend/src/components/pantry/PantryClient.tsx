...
export function PantryClient({ initialItems }: Props) {
  ...
  async function handleParse() {
    ...
    try {
      const data = await apiClient.post<{
        items: { name: string; quantity: number; unit: string | null; category: string | null }[]
      }>('/api/pantry/parse', { text: quickAddText })
      setParsedItems(
        data.items.map((i) => ({
          name: i.name,
          quantity: i.quantity,
  ...
  async function handleSave() {
    ...
    try {
      if (editItem) {
        const data = await apiClient.put<{ item: PantryItem }>(`/api/pantry/${editItem.id}`, {
          name: form.name,
        ...
        const data = await apiClient.post<{ items: PantryItem[] }>('/api/pantry', toAdd)
        setItems((prev) => [...prev, ...data.items])
      } else {
        const data = await apiClient.post<{ items: PantryItem[] }>('/api/pantry', {
          name: form.name,
          quantity: Number(form.quantity),
          unit: form.unit || null,
          category: form.category || null,
  ...
  async function handleDelete(id: string) {
    setDeleting(id)
    try {
      await apiClient.delete(`/api/pantry/${id}`)
            ...
            <div className="p-5">
              ...
              <div className="flex gap-2 mt-5">
                ...
                <button
                  ...
                  disabled={
                    saving ||
                    (!editItem && !quickAddMode && !form.name) ||
                    (quickAddMode && parsedItems.length === 0)
                  }
