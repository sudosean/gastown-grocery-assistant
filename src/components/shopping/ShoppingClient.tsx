...
import { GroceryList, GrocerySection, GroceryItem } from '@/app/api/grocery-list/generate/route'
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch('/api/grocery-list/generate', {
        ...
        body: JSON.stringify({ mealPlan, pantryItems: pantryNames }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Generation failed')
      setGroceryList(data as GroceryList)
    } catch (err) {
